...
export default function PRSStudioPage() {
  ...
  const { data: historicalDocs, isLoading: docsLoading, refetch: refetchDocs } = useQuery({
    queryKey: ["historicalPrsDocs", projectId],
    queryFn: async () => { if (!projectId) return []; const res = await api.get(`/documents/project/${projectId}`); return res.data.filter((doc: any) => doc.doc_type === "prs"); },
    enabled: !!projectId,
  });
  ...
  const generateByTextMutation = useMutation({
    mutationFn: async () => {
      const docRes = await api.post(`/documents/project/${projectId}`, { title: "AI Generated PRS", doc_type: "prs", version: 1, content: "Initial PRS Outline" });
      const genRes = await api.post(`/documents/${docRes.data.id}/generate-prs?project_id=${projectId}`, { context_text: contextText });
      return genRes.data;
    },
    onSuccess: (data) => { setTaskId(data.task_id); setActiveDocId(data.document_id); setStatus("POLLING"); },
    onError: (err: any) => { setStatus("ERROR"); setErrorMsg(err.response?.data?.detail || "Failed to trigger PRS generation."); },
  });
  ...
  const generateByFileMutation = useMutation({
    mutationFn: async () => {
      ...
      formData.append("title", selectedFile?.name || "Uploaded PRS Source");
      const res = await api.post(`/documents/project/${projectId}/generate-prs-from-file`, formData, { headers: { "Content-Type": "multipart/form-data" } });
      return res.data;
    },
    onSuccess: (data) => { setTaskId(data.task_id); setActiveDocId(data.document_id); setStatus("POLLING"); },
    onError: (err: any) => { setStatus("ERROR"); setErrorMsg(err.response?.data?.detail || "Failed to trigger PRS generation."); },
  ...
  return (
    <StudioPageLayout
      title="PRS Studio"
      subtitle="Transform product vision into structured Product Requirement Specifications."
      accentColor="#AF52DE"
      projects={projects}
      projectId={projectId}
      ...
      contextPlaceholder="Describe the product vision, target audience, key features, and user personas…"
      quickTags={[{ label: "User-Centric" }, { label: "Market-Driven" }]}
      isGenerating={isGenerating}
      onGenerate={handleGenerate}
      generateLabel="Generate PRS"
      status={status}
      result={result}
      errorMsg={errorMsg}
      errorRef={errorRef}
      ...
      outputSuccessLabel="PRS Generated Successfully"
      onExport={() => downloadAsDoc(result, "Architect_PRS_Output")}
